...
func (ob *OrderBook) ProcessMarketOrder(account string, side Side, quantity decimal.Decimal) (done []*Order, partial *Order, partialQuantityProcessed, quantityLeft decimal.Decimal, err error) {
	if quantity.Sign() <= 0 {
	...
	for quantity.Sign() > 0 && sideToProcess.Len() > 0 {
		bestPrice := iter()
		ordersDone, partialDone, partialProcessed, quantityLeft := ob.processQueue(account, bestPrice, quantity)
...
//	partialQuantityProcessed - if partial order is not nil this result contains processed quatity from partial order
func (ob *OrderBook) ProcessLimitOrder(account string, side Side, orderID string, quantity, price decimal.Decimal) (done []*Order, partial *Order, partialQuantityProcessed decimal.Decimal, err error) {
	...
	for quantityToTrade.Sign() > 0 && sideToProcess.Len() > 0 && comparator(bestPrice.Price()) {
		ordersDone, partialDone, partialQty, quantityLeft := ob.processQueue(account, bestPrice, quantityToTrade)
		done = append(done, ordersDone...)
		partial = partialDone
	...
	if quantityToTrade.Sign() > 0 {
		o := NewOrderWithOwner(orderID, account, side, quantityToTrade, price, time.Now().UTC())
		if len(done) > 0 {
		...
		done = append(done, NewOrderWithOwner(orderID, account, side, quantity, totalPrice.Div(totalQuantity), time.Now().UTC()))
	}
	return
}

func (ob *OrderBook) processQueue(account string, orderQueue *OrderQueue, quantityToTrade decimal.Decimal) (done []*Order, partial *Order, partialQuantityProcessed, quantityLeft decimal.Decimal) {
	...
	for orderQueue.Len() > 0 && quantityLeft.Sign() > 0 {
		...
		if quantityLeft.LessThan(headOrder.Quantity()) {
			partial = NewOrderWithOwner(headOrder.ID(), account, headOrder.Side(), headOrder.Quantity().Sub(quantityLeft), headOrder.Price(), headOrder.Time())
...
func (ob *OrderBook) GerOrders() map[string]*list.Element {
	return ob.orders
}
